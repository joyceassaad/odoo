    ...
    def _prepare_survey_data(self, survey_sudo, answer_sudo, **post):
        ...
        if survey_sudo.participants_can_see_votes:
            ...
            data.update({'answers_count': {
                str(r['question'].id): {str(s['suggested_answer'].id): s.get('count') for s in r.get('table_data', [])
                                        if isinstance(s, dict) and 'suggested_answer' in s.keys()}
                for r in stats_array if isinstance(r, dict) and 'question' in r.keys()}})
